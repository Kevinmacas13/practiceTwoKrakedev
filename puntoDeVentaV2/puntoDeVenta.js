calcularValorTotal = function () {
  ...
  if (
    esProductoValido(cmpNombreProducto) &
    esCantidadValida(cmpCantidad) &
    esPrecioValido(cmpPrecioProducto)
  ) {
    ...
    valorDescuento = calcularDescuentoPorVolumen(valorSubtotal, cmpCantidad);
    ...
    cmpResumen.innerText = `Valor a pagar por ${cmpCantidad} ${cmpNombreProducto} con ${valorDescuento}$ de descuento: USD ${valorTotal.toFixed(
      2
    )}`;
...
esProductoValido = function (cmpNombreProducto) {
  if (cmpNombreProducto == "") {
    mostrarTexto("lblError1", "CAMPO OBLIGATORIO");
    return false;
  } else {
    if (cmpNombreProducto.length < 10) {
      mostrarTexto("lblError1", "");
      return true;
    } else {
      mostrarTexto("lblError1", "TEXTO MUY LARGO");
      return false;
    }
  }
};

esCantidadValida = function (cmpCantidad) {
  if (isNaN(cmpCantidad)) {
    mostrarTexto("lblError2", "CAMPO OBLIGATORIO");
    return false;
  } else {
    if (cmpCantidad >= 0 && cmpCantidad <= 100) {
      mostrarTexto("lblError2", "");
      return true;
    } else {
      mostrarTexto("lblError2", "CANTIDAD NO VÁLIDA");
      return false;
    }
  }
};

esPrecioValido = function (cmpPrecioProducto) {
  if (isNaN(cmpPrecioProducto)) {
    mostrarTexto("lblError3", "CAMPO OBLIGATORIO");
    return false;
  } else {
    if (cmpPrecioProducto >= 0 && cmpPrecioProducto <= 50) {
      mostrarTexto("lblError3", "");
      return true;
    } else {
      mostrarTexto("lblError3", "PRECIO NO VÁLIDO");
      return false;
    }
  }
};
